...
僕自身きちんと実装を読んだわけではないのですが、`tf.GradientTape` の処理はおそらく誤差逆伝播法のためのフォーマットで、ニューラルネットの順伝播の際の各層での出力値を記録しておくための処理と思われます。
...
$\blacksquare$ **練習問題2:** 分類モデルを「だます」ような入力値の微小な変更を **敵対的摂動 (adversarial perturbation)** などと言います。いろいろなやり方がありますが、たとえば適当に小さな $\delta \mathbf{x}$ から初めて
...
#### 訓練ステップ
`train_step(self)` が呼び出されています。ですのでこのメソッドを上書きすれば、訓練ループを自分で書かずに、そのまま `model.fit()` を使えるようになります。基本は以下のフォーマットで良いようです：
```python
class MyModel(keras.Model):
    ...
    def train_step(self, data):
        ...
        return {"loss": loss_value} # 返り値を辞書オブジェクトにする
```

使用の際はたとえば以下のようにするなど考えられます：
...
$\blacksquare$ **練習問題3:** 手書き文字認識をkeras+tfで書く
> [!TIP]
> <details open>
> <summary>解答例</summary>
> 
...
$\blacksquare$ **練習問題4:** 手書き文字認識をtorchで書く
> [!TIP]
> <details open>
> <summary>解答例</summary>
> 
...
$\blacksquare$ **練習問題?:** 手書き文字認識をflaxで書く
> [!TIP]
> <details open>
> <summary>解答例</summary>
> 
...
$\blacksquare$ **練習問題?:** score-matching
> [!TIP]
> <details open>
> <summary>解答例</summary>
> 
